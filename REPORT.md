...
Классический детерминированный подход требует введения априорных стабилизирующих функционалов (квадратичное сглаживание по соседям, градиентная регуляризация), формирования СЛАУ высокой размерности и подбора коэффициента сглаживания $\gamma$, что сопряжено со значительными вычислительными затратами и сглаживанием резких геологических контактов.
...
5. **Классическая инверсия**: реализация квадратичного сглаживания по 4-связной сетке соседей ($\gamma$-регуляризация) с автоматическим масштабированием диагонали оператора для прямого сопоставления результатов.
...
- **Классический метод сглаживания**: поиск минимума параметрического сглаживающего функционала:
  $$\Phi(\boldsymbol{\lambda}) = \|L\boldsymbol{\lambda} - \mathbf{S}_{\text{obs}}\|_2^2 + \gamma_{\text{eff}} \sum_{k=1}^K \sum_{m \in \mathcal{N}(k)} (\lambda_k - \lambda_m)^2 \to \min$$
  где $\gamma$ штрафует градиентные перепады между соседними ячейками $\mathcal{N}(k)$ (пространственное лапласовское сглаживание).  
...
Для сопоставления результатов нейросетевой инверсии реализован классический детерминированный алгоритм инверсии со сглаживанием по 4-связной сетке соседей ($\gamma$-регуляризация).

Функционал невязки имеет вид:
$$\Phi(\boldsymbol{\lambda}) = \|L\boldsymbol{\lambda} - \mathbf{S}_{\text{obs}}\|_2^2 + \sum_{k=1}^K \gamma_k \sum_{m \in \mathcal{N}(k)} (\lambda_k - \lambda_m)^2 \to \min$$
где:
...
$$(L^T L + C(\gamma)) \boldsymbol{\lambda} = L^T \mathbf{S}_{\text{obs}}$$
где:
...
2. **Панель управления экспериментом**:
   ...
   - Запуск классического алгоритма («Решить классическим алгоритмом») с заданием параметра сглаживания $\gamma$;
...
#### Статистика обучения:
- **Время обучения**: 45.00 секунд (80 эпох на стандартном многоядерном CPU);
- **Начальная ошибка (Эпоха 1)**: Train MSE = 0.033706, Val MSE = 0.036253, Val MAE = 0.1131;
- **Промежуточная сходимость (Эпоха 40)**: Train MSE = 0.030584, Val MSE = 0.035412, Val MAE = 0.1086;
- **Лучшая валидационная эпоха (Эпоха 77)**: Train MSE = **0.029504**, Val MSE = **0.034826**, Val MAE = **0.1075**;
- **Итоговая сходимость (Эпоха 80)**: Train MSE = 0.030061, Val MSE = 0.035608, Val MAE = 0.1082;
- **Анализ сходимости и переобучения**: Разрыв между кривыми потерь Train MSE (~0.0295) и Val MSE (~0.0348) минимален и стабилен на протяжении всех 80 эпох. Обе кривые синхронно снижаются и выходят на пологое асимптотическое плато без признаков расхождения, что подтверждает полное отсутствие эффекта переобучения (overfitting). Слои `Dropout(p=0.15)` и `BatchNorm1d` обеспечили надежную регуляризацию внутреннего признакового пространства.
...
|---|---|---|---|---|
| **Обученная ИНС (PyTorch MLP)** | **0.0398** | **0.001201** | **24.42%** | **~1 мс** |
| **Классическая $\gamma$-регуляризация ($\gamma=0.05$)** | 0.0807 | 0.002341 | 40.43% | ~8 мс |
| **Классическая без регуляризации ($\gamma=0$)** | > 0.4500 (осцилляции) | $0.000012$ | 0.8% | ~6 мс |

#### Анализ результатов:
1. **Локализация геометрии объекта**: ИНС восстановила контрастный блок с четкими краями строго на проектной глубине ($z = 200\dots 400$ м, $x = 700\dots 1100$ м). Пиковое значение намагниченности достигло 0.86–0.90 при минимальной интегральной ошибке по всей сетке MAE = **0.0398** (погрешность менее 4%).
2. **Особенности классического метода**: Классическая $\gamma$-регуляризация ($\gamma=0.05$), минимизируя квадратичную разность соседних ячеек, неизбежно размывает аномалию во все стороны (эффект диффузного пятна). Пиковая намагниченность снижается до ~0.53, а вмещающие фоновые ячейки завышаются до 0.12–0.15. В итоге MAE классической инверсии (0.0807) оказалась в 2 раза хуже, чем у нейросети (0.0398).
3. **Невязка поля**: Обученная ИНС показала норму невязки $\|e\|_2 = 0.001201$ и относительную невязку $R_{sq} = 24.42\%$, превзойдя классическую регуляризацию ($\|e\|_2 = 0.002341$, $R_{sq} = 40.43\%$). Сеть эффективно отфильтровала 1% аддитивный шум, сформировав гладкий физичный отклик без переподгонки под высокочастотные флуктуации.
...
#### Анализ результатов:
- **ИНС**: MAE модели = **0.1147**, норма невязки поля $\|e\|_2 = 0.001864$, $R_{sq} = 46.88\%$. Нейросеть безошибочно определила пространственный угол падения пласта (~$45^\circ$) и зафиксировала верхнюю кромку залегания на глубине 100 м.
- **Классическая $\gamma$-регуляризация ($\gamma=0.03$)**: MAE модели = **0.1434**, $\|e\|_2 = 0.001715$, $R_{sq} = 33.18\%$. Классический метод с изотропным оператором Лапласа одинаково штрафует производные по осям $Ox$ и $Oz$, размывая диагональные геологические контакты и замещая наклонную структуру субвертикальным расплывчатым пятном.
...
#### Анализ результатов:
- **Разделение по латерали**: MAE модели = **0.1431**. ИНС отчетливо разрешает оба объекта, сохраняя межобъектное безрудное пространство с околонулевой намагниченностью ($\lambda \approx 0.01\dots 0.03$).
- **Разделение по глубине**: MAE модели = **0.1954**. Верхний приповерхностный объект локализован со 100% точностью формы и амплитуды. Нижний объект, находящийся в зоне резкого затухания дипольного поля ($1/r^3$), восстановлен в виде сглаженной области пониженной интенсивности ($\lambda \approx 0.40\dots 0.45$). Это поведение полностью соответствует физическим пределам магниторазведки, описанным в разделе 3.4 учебного пособия.
...
- **Шум 1%**: MAE = **0.0398**, $\|e\|_2 = 0.001201$, $R_{sq} = 24.42\%$, четкие границы тела, нулевой фон;
- **Шум 5%**: MAE = **0.0476**, $\|e\|_2 = 0.002822$, $R_{sq} = 40.70\%$, геометрия тела сохраняется стабильной, слабое сглаживание по углам;
- **Шум 10%**: MAE = **0.0615**, $\|e\|_2 = 0.006148$, $R_{sq} = 71.54\%$, центр тяжести и глубина тела определены надежно, ложных артефактов на краях профиля не зафиксировано.

#### Физический вывод:
При увеличении зашумленности наблюдений в 10 раз (с 1% до 10%) ошибка восстановления распределения намагниченности MAE возросла всего на **0.0217** (с 0.0398 до 0.0615). Обученная многослойная нейросеть с батч-нормализацией и регуляризацией Dropout выступает в качестве эффективного проекционного фильтра, отсекающего некоррелированные высокочастотные стохастические гармоники.
...
1. **Сплайн-аппроксимация**: Сглаживающий кубический сплайн Эрмита с параметром регуляризации $\alpha_{\text{spline}} = 10^{-3}$ непрерывно проинтерполировал редкие замеры и отфильтровал 10% шумовые выбросы, восстановив гладкую кривую $B_x(x)$ на 40 равномерных узлах с максимальным приближением к теоретическому сигналу.
2. **Инверсия**: Подача редуцированного сплайном вектора из 40 узлов на вход ИНС позволила восстановить геомагнитную модель с MAE = **0.2374**, надежно локализовав глубинное положение и геометрический центр тела даже в условиях существенной разреженности исходных наблюдений.
...
| **Помехоустойчивость** | Сильная чувствительность: требует ручной подгонки $\gamma$ | **Высокая устойчивость** благодаря генерализации на зашумленных данных |
...
4. **Спроектирована и обучена глубокая нейронная сеть**:
   - Полносвязная архитектура (40 $\to$ BatchNorm $\to$ 400 $\to$ ReLU $\to$ Dropout $\to$ 400 $\to$ ReLU $\to$ Dropout $\to$ 200 $\to$ Sigmoid) со 258 тысячами параметров продемонстрировала высокую скорость обучения (45.00 с за 80 эпох на CPU) и сошлась к минимальной ошибке Val MAE = **0.1075** (Val MSE = **0.0348**) без признаков переобучения.

5. **Проведено экспериментальное сопоставление подходов**:
   - На контрольной модели из ЛР 1 нейросеть превзошла классическую $\gamma$-регуляризацию по точности восстановления намагниченности (MAE = **0.0398** против 0.0807), обеспечив в 2 раза более четкую локализацию границ и меньшую невязку поля.
   - Нейросеть показала превосходство в локализации асимметричных наклонных пластов (MAE = 0.1147 против 0.1434) и разделении систем из нескольких тел.
   - Подтверждена высокая помехоустойчивость ИНС при уровнях шума до 10% (MAE = 0.0615), а также надежность совместной работы связки «Препроцессор (сплайн) + ИНС» при нерегулярных полевых наблюдениях (MAE = 0.2374).
